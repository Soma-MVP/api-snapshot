...
public class SearchEntityDocument {
	...
	public SearchEntityDocument(Item item, Boolean isFriend, Boolean isPromoted) {
		...
		this.isPromoted = isPromoted == null ? 0 : isPromoted ? 1 : 0;
	...
	public SearchEntityDocument(Document document) {
		...
		if(fieldNames.contains("isFriend")) {
			this.isFriend = document.getOnlyField("isFriend").getText().equals("1") ? 1 : 0;
		} else {
			this.isFriend = 0;
		}
		
		if(fieldNames.contains("isPromoted")) {
			this.isPromoted = document.getOnlyField("isPromoted").getText().equals("1") ? 1 : 0;
		} else {
			this.isPromoted = 0;
		}
...
//		logger.info("Creating document with id = " + getId());
		...
		if (isFriend != null) {
			builder = builder
					.addField(Field.newBuilder().setName("isPromoted").setText(isPromoted.toString()));
		}
		if (location != null) {
